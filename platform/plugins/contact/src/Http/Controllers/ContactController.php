...
class ContactController extends BaseController
{
    ...
    public function postReply(
        ...
        if($request->file('attachment')!=null){
            ...
            $targetDir = url('/').'/storage/contact_reply/';
            $a=$targetDir;
            $targetFilePath = $targetDir . $fileName;
            $fileType = pathinfo($targetFilePath,PATHINFO_EXTENSION);
            $request->file('attachment')->move(public_path('storage').'/contact_reply/',$fileName);
            $attachment=public_path('storage').'/contact_reply/'.$fileName;
            
            
        $data=['msg'=>$request->input('message')];
        Mail::send('mailattach',$data,function($messages) use ($user,$attachment){ 
            $messages->to($user);
