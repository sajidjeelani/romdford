...
if (!function_exists('get_flashSale_products')) {
    /**
     * @return array
     */
    function get_flashSale_products()
    {
        $flashSale = DB::table('ec_flash_sale_products')->pluck('product_id');
        return $flashSale;
    }
}

if (!function_exists('get_category_discount')) {
    /**
     * @return array
     */
    function get_category_discount(int $id)
    {
        $discountId = DB::table('ec_discount_category')->where('category_id',$id)->pluck('discount_id');
        $current_date_time = new DateTime();
        $current_date = $current_date_time->format('Y-m-d');
        foreach ($discountId as $discount_id) {
            if (is_null($discount_id)) {
                  continue;
                } 
            else {
                  $discount = DB::table('ec_discounts')->where('id',$discount_id)->get();
                  foreach($discount as $d)
                    {
                       if($d->end_date==null||$d->end_date>$current_date||$d->product_quantity>$d->total_used){
                           if($d->type=="promotion"){
                            $dis= get_object_vars($d);
                            return $dis;
...
if (!function_exists('get_categories_products')) {
    /**
     * @param int $id
     * @return Product $products
     */
    function get_categories_products($categoryId)
    {
        
        $product = DB::table('ec_product_category_product')->where('category_id',$categoryId)->pluck('product_id')->toArray();
        ...
        $params['condition'][] = ['ec_products.id', 'IN', $product];
...
if (!function_exists('get_related_products')) {
     ...
     * @return array
     */
    function get_related_products($product, $limit = 16)
    {
        $params = [
            'condition' => [
                'ec_products.status'       => BaseStatusEnum::PUBLISHED,
