        ...
        <div class="flexbox-content">
            <div class="wrapper-content">
                <div class="pd-all-20 ws-nm">
                    <div id="discount-id" style="display:none">{{discount}}</div>
                    <label id="title-product-main text-no-bold" class="title-product-main text-no-bold"><span
                            v-if="!is_promotion">{{ __('Create coupon code')}}</span><span v-if="is_promotion">{{ __('Create discount promotion') }}</span></label>
                    <a href="#" id="generate" class="btn-change-link float-right" v-on:click="generateCouponCode($event)"
                       v-show="!is_promotion">{{ __('Generate coupon code')}}</a>
                    <div class="form-group mt15 mb0">
                        <input type="text" id="code" class="next-input coupon-code-input" name="code" v-model="code"
                               v-show="!is_promotion" value="code">
                ...
                <div class="pd-all-20 border-top-color">
                    ...
                    <div class="ui-select-wrapper width-200-px-rsp-768 mt15">
                        <select class="ui-select" id="select-promotion" name="type" v-model="type"
                            ...
                            <option id="promotion" value="promotion">{{ __('Promotion')}}</option>
                        </select>
                    ...
                    <div class="form-group mb0 mt15" v-show="!is_promotion">
                        <label>
                            <input type="checkbox" class="hrv-checkbox" name="is_unlimited" id="is_unlimited" value="1"
                    ...
                    <div class="form-inline form-group discount-input mt15 mb0 ws-nm">
                        ...
                        <div class="inline width20-rsp-768 mb5">
                            <div class="next-input--stylized">
                                <input type="text" class="next-input next-input--invisible" name="value"
                                       v-model="discount_value" value="discount_value" autocomplete="off" placeholder="0">
                        ...
                        <div v-show="type_option !== 'shipping' && type_option">
                            ...
                            <div class="inline mb5" id="div-select-category"
                                 v-if="target === 'category' && type_option !== 'shipping'">
                                <div class="drop-select-search drop-control dropdown dropdown-collection">
                                    <input type="hidden" name="categories" v-model="category_id">
                                    <button class="btn btn-secondary dropdown-toggle" type="button"
                                            data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"
                                            @click="getCategories()">
                                        <span class="overflow-title max-250 p-r15">{{ category_name }}</span>
                                    </button>
                                    <div class="dropdown-menu">
                                        <div class="has-loading" v-show="loading">
                                            <i class="fa fa-spinner fa-spin"></i>
                                        ...
                                        <a v-show="!loading" class="dropdown-item"
                                           @click="handleSelectCategory(category)" href="#"
                                           v-for="category in categories" v-bind:value="category.id">{{ category.name }}</a>
                                    </div>
                                </div>
                            </div>
            ...
            <div class="wrapper-content">
                ...
                <div class="pd-all-10-20">
                    <label class="next-label disable-input-date-discount">
                        <input type="checkbox" class="hrv-checkbox" name="unlimited_time" id="unlimited_time" value="1"
...
<script>
    ...
    export default {
        ...
        mounted: function () {
            ...
            if(discount!=0){
                axios.get('/admin/discounts/getData/'+discount)
                        .then(res => {
                            let date="";
                            document.getElementById('code').disabled=true;
                            document.getElementById('generate').remove();
                            document.getElementById('title-product-main text-no-bold').innerHTML="<span>Edit Coupon</span>";
                            document.getElementById('promotion').remove();
                            document.getElementById('select-promotion').disabled=true;
                            context.code=_.first(res.data).code;
                            context.title=_.first(res.data).title;
                            if(_.first(res.data).start_date!=null){
                                date=_.first(res.data).start_date.split(" ");
                                context.start_date=date[0];
                                context.start_time=date[1];
                                
                            }
                            if(_.first(res.data).end_date!=null){
                            date=_.first(res.data).end_date.split(" ");
                            context.end_date=date[0];
                            context.end_time=date[1];
                            document.getElementById("unlimited_time").value="0";
                            document.getElementById("unlimited_time").checked=false;
                            }
                            context.can_use_with_promotion=_.first(res.data).can_use_with_promotion;
                            context.customer_id=_.first(res.data).customer_id;
                            context.product_id=_.first(res.data).product_id;
                            context.discount_on=_.first(res.data).discount_on;
                            context.min_order_price=_.first(res.data).min_order_price;
                            context.product_quantity=_.first(res.data).product_quantity;
                            context.quantity=_.first(res.data).quantity;
                            context.target=_.first(res.data).target;
                            document.getElementById("select-offers").value=context.target;
                            
                            context.type=_.first(res.data).type;
                            context.type_option=_.first(res.data).type_option;
                            context.discount_value=_.first(res.data).value;
        ...
        methods: {
            ...
            getCategories: function () {
                ...
                if (_.isEmpty(context.product_collections)) {
                    context.loading = true;
                    axios
                        .get('/admin/discounts/getCategories')
                        .then(res => {
                            console.log(res.data);
                            context.categories = res.data;
                            if (!_.isEmpty(res.data)) {
                                context.category_id = _.first(res.data).id;
                            }
                            context.loading = false;
                        })
                        .catch(res => {
