...
class PublicCartController extends Controller
{
     ...
     * @return array|BaseHttpResponse|RedirectResponse
     */
    public function addGiftWrap($amount, BaseHttpResponse $response){
        
        $token = OrderHelper::getOrderSessionToken();
        
        
        //get current session cart items
        $user = DB::table('ec_orders')->where('token', $token)->first();
        $giftWrap = $user->gift_wrap;
        
        
        //remove gift wrapper amount set column to 0
        if($amount == 'remove'){
        $totalAmount = ($user->amount - $giftWrap);
        $affected = DB::table('ec_orders')
              ->where('token', $token)
              ->update(['gift_wrap' => 0, 'amount'=>$totalAmount]);
        return $response->setMessage(__('Gift Wrapper Removed.'));
            
        }else{
        //Add gift wrapper amount
        $totalAmount = ($user->amount + $amount);
        $affected = DB::table('ec_orders')
              ->where('token', $token)
              ->update(['gift_wrap' => $amount, 'amount'=>$totalAmount]);
        return $response->setMessage(__('Gift Wrapper Added.'));
