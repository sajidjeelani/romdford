...
class PublicCheckoutController
{
    ...
    public function getCheckout(
        ...
        if (is_plugin_active('marketplace')) {
            ...
            $shippingData = [
                ...
                'state'       => Arr::get($sessionCheckoutData, 'company_name'),
    ...
    public function postCheckout(
        ...
        $shippingAmount = 0;
        $address=Arr::get($sessionData, 'address').', '.Arr::get($sessionData, 'address1').', '.Arr::get($sessionData, 'aaddress');
        
        if ($request->has('shipping_method') && !get_shipping_setting('free_ship',
                $request->input('shipping_method'))) {
            $shippingData = [
                'address'     => $address,
                'company_name' =>Arr::get($sessionData, 'company_name'),
        ...
        $amount = Cart::instance('cart')->rawTotal() + Cart::instance('cart')->rawGift() + (float)$shippingAmount - $promotionDiscountAmount - $couponDiscountAmount;
        ...
        if ($order) {
            ...
            DB::table('ec_order_addresses')
              ...
              ->update(['company_name' => Arr::get($sessionData, 'company_name'),'address' => Arr::get($sessionData, 'address'),'address1'=>Arr::get($sessionData, 'address1'),'aaddress'=>Arr::get($sessionData, 'aaddress')]);
            return $response
    ...
    public function getPayPalStatus(
        ...
        $token = $payPalPaymentService->afterMakePayment($request);
        return $response
