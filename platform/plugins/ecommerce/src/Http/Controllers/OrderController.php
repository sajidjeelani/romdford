...
class OrderController extends BaseController
{
     ...
     * @return BaseHttpResponse
     */
    public function getCourier(Request $request, BaseHttpResponse $response)
    {
        $couriers=DB::table('ec_courier')->get();
     ...
     * @return BaseHttpResponse
     */
    public function courierDetails($id,Request $request, BaseHttpResponse $response)
    {
        $result=DB::table('ec_orders')
    ->where(['id' => $id])
    ->update(['courier_id' => $request->courierName,
             'tracking_number' => $request->trackingnum]);
             if($result)
       return $response->setMessage("Success");
       else
       $response->setError()->setMessage("Error");
    }
    ...
    public function postCreateShipment(
        ...
        if (!$result->isError()) {
            ...
            $trackingNum=$request->input('note');
            $mailer = EmailHandler::setModule(ECOMMERCE_MODULE_SCREEN_NAME);
            if ($mailer->templateEnabled('customer_delivery_order')) {
                OrderHelper::setEmailVariables($order);
                $mailer->sendUsingTemplate(
                    'customer_delivery_order',
                    $order->user->email ?: $order->address->email,['trackingNum'=>$order->tracking_number]
                );
            }
