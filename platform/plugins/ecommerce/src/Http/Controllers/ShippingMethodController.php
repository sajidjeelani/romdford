...
class ShippingMethodController extends BaseController
{
     ...
     * @throws Exception
     */
    public function addCourier(Request $request, BaseHttpResponse $response)
    {
        $name=DB::table('ec_courier')->where('courier_name', $request->input('courier'))->first();
        
        if($name)
            $name=$name->courier_name;
        else
            $name=null;
        if($name!=$request->input('courier')){
            $result=DB::table('ec_courier')->insertOrIgnore([
                'courier_name' => $request->courier
            ]);
        return $response->setMessage("Added Successfully");
        }
        else
        return $response->setError()->setMessage("Already Exist");
     ...
     * @throws Exception
     */
    public function deleteCourier(Request $request, BaseHttpResponse $response)
    {
        $result=DB::table('ec_courier')->where('courier_name', $request->courier)->delete();
        if($result==1){
            
            return $response->setMessage("Deleted Successfully");
        }
        return $response->setError()->setMessage($result);
