    ...
    public function execute(string $coupon, $sessionData = [], $cartData = [], $prefix = '')
    {
        ...
        if ($discount->target === 'customer') {
            ...
            if (!auth('customer')->check() ||
                ...
                return [
                    'error'   => true,
                    'message' => "Customer Not Applicable",
                ];
            }
        }

        if (!$discount->can_use_with_promotion && (float)Arr::get($sessionData, 'promotion_discount_amount') && ($discount->title !== "NewsLetter Registration"  || $discount->title !== "Welcome Offer" )) {
            return [
                'error'   => true,
            ...
            switch ($discount->type_option) {
                case 'amount':
                    switch ($discount->target) {
                        ...
                        case 'category':
                            $val=false;
                            $categoryID=DB::table('ec_discount_category')->where('discount_id', $discount->id)->pluck('category_id');
