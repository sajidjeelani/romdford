...
class OrderHelper
{
    ...
    {
        $results = DB::table('ec_courier')
            ->select('courier_name')
            ->where('id', '=', $order->courier_id)
            ->get();
        if ($results->isEmpty()) {
            $courierName = ''; // Set a default value
        } else {
            $courierName = $results[0]->courier_name;
        }
        ...
        if ($orderAddress) {
            ...
            // Alternatively, you can print all values using var_dump or dd
    ...
    public function sendOrderConfirmationEmail($order, $saveHistory = false)
    {
        try {
            ...
            if ($mailer->templateEnabled('order_confirm')) {
                ...
                EmailHandler::send(
                    $mailer->getTemplateContent('order_confirm'),
                    $mailer->getTemplateSubject('order_confirm'),
