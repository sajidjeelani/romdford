                        ...
                        @foreach($addresses as $address)
                            <tr>
                                <td style="white-space: inherit;">
                                    <p>{{ $address->name }}, {{ $address->company_name }} , {{ $address->address }}, {{ $address->address1 }},{{ $address->aaddress }}, {{ $address->city }}, {{ $address->state }}@if (count(EcommerceHelper::getAvailableCountries()) > 1), {{ $address->country_name }} @endif @if (EcommerceHelper::isZipCodeEnabled()), {{ $address->zip_code }} @endif - {{ $address->phone }}</p>
                                </td>
